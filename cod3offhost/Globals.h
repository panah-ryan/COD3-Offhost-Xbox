...
VOID DbgPrint(const CHAR* strFormat, ...)
{
	...
	CHAR buffer[0x1000];
	vsprintf_s(buffer, strFormat, pArgList);
	va_end(pArgList);
...
char* XamShowKeyboardUI(LPCWSTR titleText, LPCWSTR descriptionText, int* error)
{
	XOVERLAPPED overlapped;
	WCHAR keyboardText[1024];
	static char buffer[1024];
	ZeroMemory(&overlapped, sizeof(XOVERLAPPED));
	XShowKeyboardUI(0, 0, L"", titleText, descriptionText, keyboardText, 0x20, &overlapped);
	while (!XHasOverlappedIoCompleted(&overlapped))
		Sleep(50);
	*error = overlapped.dwExtendedError;
	wcstombs(buffer, keyboardText, 0x20);
	return buffer;
}
